...
class Cache {
    ...
    hasInner(url, config, { isAsync = false } = {}) {
        ...
        return atma_io_1.File[isAsync ? 'exists' : 'existsAsync'](meta.file);
    }
    has(url, config) {
    ...
    async get(url, config) {
        ...
        return await new atma_io_1.File(`./cache/squery/${meta.file}`, { cached: false }).readAsync();
    }
    ...
    flushMeta() {
        ...
        setTimeout(() => {
            atma_io_1.File
                .writeAsync('./cache/squery/meta.json', this.meta)
                .always(x => {
                this.isFlushDeferred = false;
            });
        }, 1000);
