...
class Cache {
    ...
    hasInner(url, config, { isAsync = false } = {}) {
        ...
        return atma_io_1.File[isAsync ? 'exists' : 'existsAsync'](`${CACHE_BASE}/${meta.file}`);
    }
    has(url, config) {
    ...
    async get(url, config) {
        ...
        let result = await new atma_io_1.File(`${CACHE_BASE}/${meta.file}`, { cached: false }).readAsync({ encoding });
        if (withCompression) {
            let str = await Compression.decompress(result);
            result = JSON.parse(str);
        }
        if (result.file != null) {
            result.body = await new atma_io_1.File(`${CACHE_BASE}/${result.file}`, { cached: false }).readAsync({ encoding });
            delete result.file;
        }
        return result;
    }
    save(url, config, resp) {
        ...
        if (!withCompression) {
            new atma_io_1.File(`${CACHE_BASE}/${file}`, { cached: false }).writeAsync(json);
            return;
        }
        let str = JSON.stringify(json);
        Compression.compress(Buffer.from(str)).then(buffer => {
            new atma_io_1.File(`${CACHE_BASE}/${file}`, { cached: false }).writeAsync(buffer);
        });
    }
    ...
    flushMeta() {
        ...
        setTimeout(() => {
            atma_io_1.File
                .writeAsync(`${CACHE_BASE}/meta.json`, this.meta)
                .always(x => {
                this.isFlushDeferred = false;
            });
        }, 1000);
...
exports.NetworkDriver = {
    ...
    load(url, config = {}) {
        ...
        if (config.cookies) {
            CookieContainer_1.cookieContainer.addCookies(config.cookies);
        }
        if (options.headers['Cookie']) {
            CookieContainer_1.cookieContainer.addCookies(options.headers['Cookie']);
        }
        let cookies = CookieContainer_1.cookieContainer.getCookies(url);
        if (cookies) {
...
class CherrioQuery extends IQuery_1.IQuery {
    ...
    static load(url, config, setts) {
        ...
        NetworkDriver_1.NetworkDriver.load(url, setts.opts).then(content => {
            let $ = CheerioUtils_1.CheerioUtils.fromHtml(content.toString());
            query.add($);
            query.resolve(query);
        });
        return query;
