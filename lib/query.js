...
;
	...
	if (isObject(_src_utils_Network) && isObject(module.exports)) {
		Object.assign(_src_utils_Network, module.exports);
		return;
	}
	_src_utils_Network = module.exports;
}());
// end:source ./ModuleSimplified.js
...
var dfr_1 = _src_utils_dfr;
var Network_1 = _src_utils_Network;
...
var Cache = /** @class */ (function () {
    ...
    Cache.prototype.get = function (url, config) {
        ...
        var response = new atma_io_1.File("./cache/squery/" + meta.file, { cached: false }).read();
        return response;
    };
    Cache.prototype.save = function (url, config, json) {
        ...
        new atma_io_1.File('./cache/squery/meta.json', { cached: false }).write(this.meta);
        new atma_io_1.File("./cache/squery/" + file, { cached: false }).writeAsync(json);
    };
    Cache.prototype.normalizeUrl = function (url, config) {
        url = url.toLowerCase();
    ...
    Cache.prototype.ensureMeta = function () {
        ...
        if (atma_io_1.File.exists(file)) {
            this.meta = atma_io_1.File.read(file);
        }
        else {
            this.meta = {};
        }
...
exports.NetworkDriver = {
    ...
    load: function (url, config) {
        ...
        var retryCount = 'retryCount' in config ? config.retryCount : 3;
        var retryTimeout = 'retryTimeout' in config ? config.retryTimeout : 1000;
        if (options.cookies) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            function doFetch() {
                fetch.fetchUrl(url, options, function (error, meta, body) {
                    ...
                    var setCookie = meta.responseHeaders['set-cookie'];
                    if (setCookie) {
                        CookieContainer_1.cookieContainer.addCookies(url, setCookie);
                    }
                    if (meta.status >= 400) {
...
var CherrioQuery = /** @class */ (function (_super) {
    ...
    CherrioQuery.prototype.textGetFn = function (node) {
        return dfr_1.dfr_resolve($(node).text());
    };
    ...
    CherrioQuery.load = function (url, config, setts) {
        var query = new CherrioQuery();
        NetworkDriver_1.NetworkDriver.load(url, setts.opts).then(function (content) {
            var $ = CheerioUtils_1.CheerioUtils.fromHtml(content.toString());
            query.add($);
            query.resolve(query);
        });
        return query;
...
exports.CheerioDriver = {
    ...
    load: function (url, config) {
        var query = CherrioQuery_1.CherrioQuery.newAsync();
        NetworkDriver_1.NetworkDriver
            .load(url, config)
            .then(function (resp) {
            var $el = CheerioUtils_1.CheerioUtils.fromHtml(resp.body.toString());
            query.add($el);
            query.resolve(query);
        }, function (error) { return query.reject(error); });
        return query;
